...
const CHANNELS = [
  ...
  },
  {
    key: 'ads',
    title: 'Ads Manager',
    description: 'Kampagnen erstellen, verwalten und optimieren',
    segment: '/ads',
    icon: '▶',
    live: false,
  },
...
export default function ProjectOverview({ params }) {
  ...
  useEffect(() => {
    if (!project) return;
    fetch(`/api/dashboard-stats?project_id=${project.id}&days=30`)
      .then((res) => res.json())
      .then((data) => {
        if (data.submissions) {
          setQuizStats({
  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">Alle Kanäle im Überblick</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {CHANNELS.map((channel) => (
          <ChannelCard key={channel.key} channel={channel} projectSlug={projectSlug} quizStats={quizStats} />
        ))}
      </div>
...
function ChannelCard({ channel, projectSlug, quizStats }) {
  const isLive = channel.live;
  const href = `/dashboard/${projectSlug}${channel.segment}`;

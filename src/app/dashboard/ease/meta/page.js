...
export default function MetaPage() {
  ...
  // Fetch campaign data when connected
  useEffect(() => {
    if (!connected) return;

    async function fetchCampaigns() {
      try {
        const res = await fetch(`/api/meta/campaigns?project_id=${PROJECT_ID}&days=${days}`);
        if (res.ok) {
          const data = await res.json();
  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div className="flex items-center gap-2 text-sm mb-1">
        ...
        <span className="text-gray-600">/</span>
        <span className="text-ease-cream font-medium">Meta Ads</span>
      </div>

      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-xl font-semibold text-ease-cream">Meta Ads</h1>
          {account?.name && (
            <p className="text-xs text-gray-500 mt-0.5">Konto: {account.name} ({account.currency})</p>
          )}
        </div>
        {connected && (
      ...
      {connected === false && (
        <ConnectPrompt
          ...
          icon={'\u25CE'}
          features={[
            'Kampagnen-Uebersicht mit Spend, Impressionen & Klicks',
            'ROAS & CPA Tracking ueber Zeit',
      ...
      {connected === true && (
        <>
          {error && (
            <div className="bg-ease-red/10 border border-ease-red/30 text-ease-red text-sm px-4 py-3 rounded-xl mb-6">
              {error}
            </div>
          )}
          ...
          {totals && (
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
              <KpiCard
                title="Ausgaben"
                value={`\u20AC${Number(totals.spend || 0).toLocaleString('de-DE', { minimumFractionDigits: 2 })}`}
                icon={'\u25CE'}
              />
              <KpiCard
                title="ROAS"
                value={`${totals.roas || '0.00'}x`}
              />
              <KpiCard
                title="CPA"
                value={`\u20AC${totals.cpa || '0.00'}`}
              />
              <KpiCard
                title="Impressionen"
                value={Number(totals.impressions || 0).toLocaleString('de-DE')}
              />
              <KpiCard
                title="Klicks"
                value={Number(totals.clicks || 0).toLocaleString('de-DE')}
              />
              <KpiCard
                title="Conversions"
                value={Number(totals.conversions || 0).toLocaleString('de-DE')}
              />
            </div>
          )}
                    ...
                    {campaigns.map((campaign) => (
                      <tr key={campaign.id} className="border-b border-ease-border/50 hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="text-right px-4 py-3">
                          <span className={parseFloat(campaign.roas) >= 1 ? 'text-ease-green' : 'text-ease-red'}>
                            {campaign.roas}x
                          </span>
                        </td>

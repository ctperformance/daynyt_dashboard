  ...
  try {
    ...
    if (!insightsRes.ok) {
      ...
      if (authResult.retry && authResult.new_token) {
        ...
        const retryRes = await fetch(`https://graph.facebook.com/v21.0/${adAccountId}/insights?fields=${fields}&time_range=${timeRange}&level=campaign&limit=200&access_token=${access_token}`);
        if (retryRes.ok) {

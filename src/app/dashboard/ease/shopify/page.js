...
export default function ShopifyPage() {
  ...
  // Fetch Shopify data when connected
  useEffect(() => {
    if (!connected) return;

    async function fetchStats() {
      try {
        const res = await fetch(`/api/shopify/stats?project_id=${PROJECT_ID}&days=${days}`);
        if (res.ok) {
          const data = await res.json();
          setStats(data);
          setShop(data.shop || {});
    ...
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          {shop?.name && (
            <p className="text-xs text-gray-500 mt-0.5">Store: {shop.name}</p>
          )}
        </div>
        {connected && (
        ...
        <>
          ...
          {stats?.totals && (
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              <KpiCard
                title="Umsatz"
                value={`\u20AC${Number(stats.totals.revenue || 0).toLocaleString('de-DE', { minimumFractionDigits: 2 })}`}
                icon={'\u2B21'}
              />
              <KpiCard
                title="Bestellungen"
                value={Number(stats.totals.orders || 0).toLocaleString('de-DE')}
              />
              <KpiCard
                title="AOV"
                value={`\u20AC${Number(stats.totals.aov || 0).toLocaleString('de-DE', { minimumFractionDigits: 2 })}`}
              />
            </div>
          )}
          ...
          {stats?.topProducts && stats.topProducts.length > 0 && (
            <div className="bg-ease-card border border-ease-border rounded-xl overflow-hidden mb-6">
              <div className="px-5 py-4 border-b border-ease-border">
                <h2 className="text-sm font-medium text-ease-cream">Top-Produkte</h2>
              </div>
                    ...
                    {stats.topProducts.map((product, i) => (
                      <tr key={product.id || i} className="border-b border-ease-border/50 hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="text-right px-5 py-3 text-gray-400">
                          {'\u20AC'}{Number(product.revenue).toLocaleString('de-DE', { minimumFractionDigits: 2 })}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {stats?.dailyTrend && stats.dailyTrend.length > 0 && (
            <div className="bg-ease-card border border-ease-border rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-ease-border">
                <h2 className="text-sm font-medium text-ease-cream">Taeglicher Verlauf</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                  ...
                  <tbody>
                    {stats.dailyTrend.slice(-14).map((day) => (
                      <tr key={day.date} className="border-b border-ease-border/50 hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="text-right px-5 py-3 text-gray-400">
                          {'\u20AC'}{Number(day.revenue).toLocaleString('de-DE', { minimumFractionDigits: 2 })}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {!stats && !error && (
            <div className="text-center py-12">
              <div className="text-gray-600 text-sm">Shopify-Daten werden geladen...</div>
            </div>
          )}
        </>
      )}

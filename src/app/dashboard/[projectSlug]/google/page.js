...
export default function GoogleAdsPage({ params }) {
    ...
    async function checkStatus() {
      try {
        ...
        if (res.ok) {
          ...
          setConnected(data.google?.connected || false);
        } else {
          setConnected(false);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    async function fetchCampaigns() {
      try {
        const res = await fetch(`/api/google/campaigns?project_id=${projectId}&days=${days}`);
        if (res.ok) {
          const data = await res.json();
  ...
  return (
    <div className="px-8 py-8 max-w-[1800px] mx-auto w-full">
      <div className="flex items-center gap-2 text-sm mb-1">
        ...
        <span className="text-gray-600">/</span>
        <span className="text-ease-cream font-medium">Google Ads</span>
      </div>

      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-xl font-semibold text-ease-cream">Google Ads</h1>
          {account?.name && (
      ...
      {connected === false && (
        <ConnectPrompt
          provider="google"
          title="Google Ads"
          description="Verbinde dein Google Ads Konto, um Kampagnen-Performance, ROAS, CPA und Conversion-Daten direkt im Dashboard zu sehen."
          icon={'\u25C9'}
          features={[

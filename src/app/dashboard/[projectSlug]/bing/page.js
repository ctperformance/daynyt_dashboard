...
export default function BingAdsPage({ params }) {
    ...
    async function checkStatus() {
      try {
        ...
        if (res.ok) {
          ...
          setConnected(data.bing?.connected || false);
        } else {
          setConnected(false);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    async function fetchCampaigns() {
      try {
        const res = await fetch(`/api/bing/campaigns?project_id=${projectId}&days=${days}`);
        if (res.ok) {
          const data = await res.json();
  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      <div className="flex items-center gap-2 text-sm mb-1">
        ...
        <span className="text-gray-600">/</span>
        <span className="text-ease-cream font-medium">Bing Ads</span>
      </div>

      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-xl font-semibold text-ease-cream">Bing Ads</h1>
          {account?.name && (
      ...
      {connected === false && (
        <ConnectPrompt
          provider="bing"
          title="Bing Ads"
          description="Verbinde dein Microsoft Advertising Konto, um Kampagnen-Performance, ROAS, CPA und Conversion-Daten direkt im Dashboard zu sehen."
          icon={'\u25A3'}
          features={[
